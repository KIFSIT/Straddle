...
namespace Straddle
{
    public partial class Stragle : Form
    {
        #region Variable
        ...
        public Stragle()
        {
            InitializeComponent();
            KeyPreview = true;
            KeyPress += new KeyPressEventHandler(Stragle_KeyPress);
        }

        void Stragle_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Escape)
            {
                AppGlobal._Strangle = null;
                Close();
            }
        }

        private void Stragle_Load(object sender, EventArgs e)
        {
        ...
        private void addRule1_Click(object sender, EventArgs e)
        {
            ...
            //if (Convert.ToString(cmbExpiry1.Text) == Convert.ToString(cmbExpiry2.Text))
            //{
            //    if (Convert.ToString(cmbSeries1.Text) == "CE")
            //    {
            ...
            //        }
            //    }
            //    else if (Convert.ToString(cmbSeries1.Text) == "PE")
            //    {
            //        if (Leg1Strike <= Leg3Strike)
            //        {
            //            MessageBox.Show("Please check Strike");
            ...
            foreach (var watchT in AppGlobal.MarketWatch.Where(x => (x.Leg1.ContractInfo.TokenNo == Convert.ToString(TokenNo))))
            {
                ...
                {
                    if (watchT.StrategyId == 2211 && watchT.Strategy == StrategyName)
                    {
           ...
           // int rowcount = 0;
            ...
            for (int i = 0; i < AppGlobal.MarketWatch.Count; i++)
            {
                ...
                if (_strategy_No > strategy_No)
                {
                    ...
                    watch.SeqaureOff = 1;
                    watch.StrategyId = 2211;
                    watch.StrategyName = "Strangle";
                    ...
                    //watch.RowData.Cells[WatchConst.RemainingDay].Value = watch.RemainingDay;
                    //watch.RowData.Cells[WatchConst.User_Rem_day].Value = watch.URem_Day;
        ...
        private void Stragle_FormClosing(object sender, FormClosingEventArgs e)
        {
            AppGlobal._Strangle = null;
        }
