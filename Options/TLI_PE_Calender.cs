...
namespace Straddle
{
    public partial class TLI_PE_Calender : Form
    {
        ...
        public TLI_PE_Calender()
        {
            InitializeComponent();
            KeyPress += new KeyPressEventHandler(TLI_PE_Calender_KeyPress);
        }

        void TLI_PE_Calender_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Escape)
            {
                AppGlobal._TLI_PE_Calender = null;
                Close();
            }
        }

        private void TLI_PE_Calender_Load(object sender, EventArgs e)
        {
        ...
        private void cmbExpiry2_SelectionChangeCommitted(object sender, EventArgs e)
        {
            string k = cmbExpiry2.Text;
            if (cmbInstrument2.Text == "OPTIDX" || cmbInstrument2.Text == "OPTSTK")
            {
                //cmbStrike1.Visible = true;
                string s1 = Convert.ToString(cmbExpiry1.Text);
                string s2 = s1.Substring(0, 4);
                ...
                string s5 = s2 + monString + s4;
                string filter1 = "InstrumentName='" + cmbInstrument2.Text + "' AND Symbol = '" + cmbSymbol2.Text + "' AND ExpiryDate = '" + s5 + "'";
                DataTable Strike = ArisApi_a._arisApi.DsContract.Tables["NseFo"];
                Strike.DefaultView.RowFilter = filter1;
                Strike.DefaultView.Sort = "StrikePrice";
                DataView table = Strike.DefaultView;
        ...
        private void TLI_PE_Calender_FormClosing(object sender, FormClosingEventArgs e)
        {
            AppGlobal._TLI_PE_Calender = null;
        }
        ...
        private void addRule1_Click(object sender, EventArgs e)
        {
            ...
            string nL1 = Convert.ToString(cmbExpiry2.Text);
            string nL2 = nL1.Substring(0, 4);
            string nL3 = nL1.Substring(4, 3);
            string nL4 = nL1.Substring(7, 2);
            int montL0 = DateTime.ParseExact(nL3, "MMM", new CultureInfo("en-US")).Month;
            System.Globalization.DateTimeFormatInfo mfiL0 = new System.Globalization.DateTimeFormatInfo();
            string monStringL0 = "";
            if (montL0 <= 9)
            {
                monStringL0 = "0" + Convert.ToString(montL0);
            }
            else
            {
                monStringL0 = Convert.ToString(montL0);
            }
            string nL5 = nL2 + monStringL0 + nL4;
            ...
            #region Expiry Check
            UInt32 checkexpiry = ArisApi_a._arisApi.DateTimeToSecond(Market.NseCm, Convert.ToDateTime(n1));
            UInt32 checkexpiry1 = ArisApi_a._arisApi.DateTimeToSecond(Market.NseCm, Convert.ToDateTime(nL1));

            if (checkexpiry >= checkexpiry1)
            {
                MessageBox.Show("Leg1 Expiry is not greater than Leg2 Expiry");
                return;
            }
            ...
            foreach (var watchT in AppGlobal.MarketWatch.Where(x => (x.Leg1.ContractInfo.TokenNo == Convert.ToString(TokenNo))))
            {
                ...
                {
                    if (watchT.StrategyId == 1114 && watchT.Strategy == StrategyName)
                    {
            ...
            AppGlobal.TLI_Strangle = AppGlobal.TLI_Strangle + 1;

            #region Strangle Leg Add
            ...
            for (int i = 0; i < AppGlobal.MarketWatch.Count; i++)
            {
                ...
                if (_strategy_No > strategy_No)
                {
                    ...
                    watch.SeqaureOff = 1;
                    watch.StrategyId = 1114;
                    watch.TLI_StrategyId = 1114;
                    watch.StrategyName = "TLI_PE_Calender";
                    ...
                    watch.Over = 0;
                    watch.Round = 0;
                    watch.TLI_UniqueId = AppGlobal.TLI_Strangle;
                    ...
                    #region Leg2
                    string strFilter2 = "";
                    strFilter2 = DBConst.InstrumentName + " = '" + cmbInstrument2.Text + "' AND " + DBConst.Symbol + " = '" + cmbSymbol2.Text + "' AND " + DBConst.ExpiryDate + " = '" + nL5 + "' AND " + DBConst.StrikePrice + " = '" + Convert.ToString(cmbStrike2.Text) + "' AND " + DBConst.Series + "= '" + cmbSeries2.Text + "'";
            ...
            TransactionWatch.ErrorMessage("StrangleStrike1 |" + cmbSymbol1.Text.ToString() + "|" + cmbStrike1.Text.ToString() + "|" + cmbExpiry1.Text.ToString() + "|" + "PE");

            #region Leg2 strike add "PE"

            #region Gui No is changing
            int flg2 = 0;
            bool NextStrategy2 = true;
            for (int i = 0; i < AppGlobal.MarketWatch.Count; i++)
            {
                MarketWatch watchfirst = new MarketWatch();
                watchfirst = AppGlobal.MarketWatch[i];
                ...
                if (_strategy_No > strategy_No)
                {
                    ...
                    #region Leg1
                    string strFilter1 = "";
                    strFilter1 = DBConst.InstrumentName + " = '" + cmbInstrument2.Text + "' AND " + DBConst.Symbol + " = '" + cmbSymbol2.Text + "' AND " + DBConst.ExpiryDate + " = '" + nL5 + "' AND " + DBConst.StrikePrice + " = '" + Convert.ToString(cmbStrike2.Text) + "' AND " + DBConst.Series + "= '" + cmbSeries2.Text + "'";
                    DataRow[] dr11 = ArisApi_a._arisApi.DsContract.Tables["NseFo"].Select(strFilter1);
                    foreach (DataRow dr in dr11)
                    {
                        watch.Leg1 = new Straddle.AppClasses.Leg();
            ...
            TransactionWatch.ErrorMessage("StrangleStrike2 |" + cmbSymbol2.Text.ToString() + "|" + cmbStrike2.Text.ToString() + "|" + cmbExpiry2.Text.ToString() + "|" + "PE");
        }
    }
}
